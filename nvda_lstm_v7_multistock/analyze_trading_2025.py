#!/usr/bin/env python3
"""
Phan tich ket qua trading tren du lieu test period (mac dinh 2025)
Hien thi chi tiet cac ngay co lenh BUY/SELL
Co the test cac nam khac nhau bang --test_year
"""
...
# Import LSTMRegressor from v7.2 module (no dependency on v4)
try:
    from nvda_lstm_v7_2_multistock import LSTMRegressor
except ImportError:
    # Fallback: try v7.1
    try:
        from nvda_lstm_v7_1_multistock import LSTMRegressor
    except ImportError:
        raise ImportError("Cannot import LSTMRegressor from v7.1 or v7.2 modules")
...
def load_test_data(data_dir, feature_cols, test_year=2025, sequence_length=30, horizon=5):
    """
    Load va prepare test data cho mot nam cu the
    V7.2: Load data truc tiep tu feature files, khong dung NVDA_MultiStock_Complete
    
    Args:
        test_year: Nam de test (mac dinh: 2025)
    """
    import glob
    
    # Load data tu feature files (same logic as v7.2 training)
    stocks = ['NVDA', 'AMD', 'MU', 'INTC', 'QCOM']
    all_dfs = []
    
    # Tim file trong data/Feature/
    ...
    for stock in stocks:
        ...
        # Them one-hot encoding
        for s in stocks:
            df[f'stock_{s}'] = 1 if s == stock else 0
        
        # Create future return target
        adj = df["Adj Close"].astype(float)
        df["future_return"] = (adj.shift(-horizon) / adj) - 1.0
        
        # Them sector features (same as training/v7.2)
        np.random.seed(42)
        ...
        df['beta_to_sox'] = cov_stock_sox / var_sox.replace(0, pd.NA)
    ...
    for df_temp in all_dfs:
        all_feature_cols.update(df_temp.columns)
    ...
    if len(df_test) == 0:
        raise ValueError(f"Khong co du lieu nam {test_year} trong dataset")
...
def print_trading_summary(results_df, test_year=2025):
    """
    In tong ket ket qua trading
    
    V7.2 Fix: Tinh Strategy Return dung cho SELL signals
    - BUY: Strategy Return = Actual_Return (mua khi giá tăng → lãi)
    - SELL: Strategy Return = -Actual_Return (bán khống khi giá giảm → lãi)
    """
    print(f"\n{'='*80}")
    print(f"TONG KET KET QUA TRADING NAM {test_year}")
    print(f"{'='*80}")
    
    # Tinh Strategy Return: BUY = Actual_Return, SELL = -Actual_Return
    results_df = results_df.copy()
    results_df['Strategy_Return'] = results_df.apply(
        lambda row: row['Actual_Return'] if row['Signal'] == 2 else (-row['Actual_Return'] if row['Signal'] == 0 else 0),
        axis=1
    )
    ...
    if len(buy_signals) > 0:
        ...
        # BUY: Strategy Return = Actual_Return
        avg_buy_return = buy_signals['Strategy_Return'].mean()
        total_buy_return = buy_signals['Strategy_Return'].sum()
        print(f"\nBUY Signals:")
        print(f"  - Win Rate: {buy_wr:.1%}")
        print(f"  - Average Return: {avg_buy_return:.4f} ({avg_buy_return*100:.2f}%)")
        print(f"  - Total Return: {total_buy_return:.4f} ({total_buy_return*100:.2f}%)")
    
    if len(sell_signals) > 0:
        sell_wr = len(sell_signals[sell_signals['Actual_Return'] < 0]) / len(sell_signals)
        # SELL: Strategy Return = -Actual_Return (bán khống ăn khi giá giảm)
        avg_sell_return = sell_signals['Strategy_Return'].mean()
        total_sell_return = sell_signals['Strategy_Return'].sum()
        print(f"\nSELL Signals:")
        print(f"  - Win Rate: {sell_wr:.1%}")
        print(f"  - Average Return: {avg_sell_return:.4f} ({avg_sell_return*100:.2f}%)")
        print(f"  - Total Return: {total_sell_return:.4f} ({total_sell_return*100:.2f}%)")
    ...
    if len(all_trades) > 0:
        ...
        # Combined: Sum of Strategy Returns (BUY + SELL với sign đúng)
        total_strategy_return = all_trades['Strategy_Return'].sum()
        print(f"\nCombined Metrics:")
        print(f"  - Combined Win Rate: {combined_wr:.1%}")
        print(f"  - Total Trading Return: {total_strategy_return:.4f} ({total_strategy_return*100:.2f}%)")
...
def create_visualizations(results_df, output_dir=None, version='v7.1', test_year=2025):
    """
    ...
    Args:
        ...
        test_year: Nam test - de hien thi trong title va filename
    """
    ...
    fig.suptitle(f'NVDA LSTM {version.upper()} - Trading Analysis {test_year}', fontsize=16, fontweight='bold')
    ...
    # Luu bieu do
    output_filename = f'trading_analysis_{test_year}_{version}.png'
...
def main():
    parser = argparse.ArgumentParser(description='Phan tich ket qua trading tren du lieu test period')
    workspace_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    ...
    parser.add_argument('--test_year', '-y', type=int, default=2025,
                       help='Nam de test (default: 2025)')
    ...
    parser.add_argument('--output', '-o', default=None,
                       help='Output CSV file name (default: trading_results_{test_year}_{version}.csv)')
    ...
    # Set default output file based on version and test year
    if args.output is None:
        args.output = f'trading_results_{args.test_year}_{args.version}.csv'
    ...
    print(f"\n{'='*80}")
    print(f"PHAN TICH KET QUA TRADING NAM {args.test_year} - {args.version.upper()}")
    print(f"{'='*80}")
    ...
    # Load full dataset (same logic as v7.2 training)
    import glob
    stocks = ['NVDA', 'AMD', 'MU', 'INTC', 'QCOM']
    all_dfs = []
    ...
    for stock in stocks:
        ...
        # Create future return target
        adj = df["Adj Close"].astype(float)
        df["future_return"] = (adj.shift(-5) / adj) - 1.0
    ...
    # Split data (same as training) - v7.2 uses sequence_length directly, not predictor
    data_splits = split_data_function(
        df_full, features, sequence_length,
        ...
        test_end=f'{args.test_year}-12-31'
    )
    ...
    # Now load test data
    print(f"\nDang load du lieu test {args.test_year}...")
    X_test, y_test, dates, df_test = load_test_data(
        args.data_dir, features, test_year=args.test_year, sequence_length=sequence_length, horizon=5
    )
    ...
    # Print summary
    print_trading_summary(results_df, test_year=args.test_year)
    ...
    try:
        fig = create_visualizations(results_df, output_dir=os.path.dirname(__file__), 
                                   version=args.version, test_year=args.test_year)
