...
def split_data_by_years(df, feature_cols, predictor,
                       pretrain_start='2015-01-01', pretrain_end='2025-12-31',
                       finetune_start='2021-01-01', finetune_end='2025-12-31',
                       val_start='2023-01-01', val_end='2024-12-31',
                       test_start='2025-01-01', test_end='2025-12-31'):
    """
    Split data theo cac giai doan:
    - Pretrain: 2015-2025 (10 nam) - all stocks
    - Fine-tune: 2021-2025 (4 nam) - all stocks
    - Validation: 2023-2024 (2 nam) - NVDA only (cho threshold optimization)
    - Test: 2025 (1 nam) - NVDA only (cho final test)
    """
    # Convert Date column to datetime if needed
    if not pd.api.types.is_datetime64_any_dtype(df['Date']):
        df['Date'] = pd.to_datetime(df['Date'])
    
    # Pretrain period: 2015-2025 (all stocks)
    pretrain_mask = (df['Date'] >= pretrain_start) & (df['Date'] <= pretrain_end)
    df_pretrain = df[pretrain_mask].copy()
    
    # Fine-tune period: 2021-2025 (all stocks)
    ...
    # Test period: 2025 (NVDA only)
    test_mask = (df['Date'] >= test_start) & (df['Date'] <= test_end) & nvda_mask
    df_test = df[test_mask].copy()
    
    print(f"\n{'='*60}")
    print("Data Split by Years:")
    print(f"{'='*60}")
    print(f"  Pretrain (2015-2025): {len(df_pretrain)} rows (all stocks)")
    print(f"  Fine-tune (2021-2025): {len(df_finetune)} rows (all stocks)")
    print(f"  Validation (2023-2024): {len(df_val)} rows (NVDA only)")
    print(f"  Test (2025): {len(df_test)} rows (NVDA only)")
...
def pretrain_model(input_size, train_loader, val_loader, epochs=200, lr=5e-4, device='cpu'):
    """
    Pretrain model tren 10 nam (2015-2025)
...
def finetune_model(model, train_loader, val_loader, epochs=100, lr=1e-4, device='cpu'):
    """
    Fine-tune model tren 4 nam (2021-2025)
...
def main():
    parser = argparse.ArgumentParser(description='NVDA LSTM v7 - Pretrain Fine-tune Strategy')
    ...
    print(f"\n{'='*60}")
    print(f"NVDA LSTM v7 - Pretrain Fine-tune Strategy (Best Practice)")
    print(f"{'='*60}")
    print(f"\nStrategy:")
    print(f"  Step 1: Pretrain trên 2015-2025 (10 năm)")
    print(f"  Step 2: Fine-tune trên 2021-2025 (4 năm) - Freeze Encoder")
    print(f"  Step 3: Freeze Model")
    print(f"  Step 4: Optimize Threshold trên 2023-2024 (Validation)")
    print(f"  Step 5: Test trên 2025 (Test Period)")
    ...
    if len(available) == 0:
        raise RuntimeError("No features available in dataset. Aborting v7 run.")

    # Split data theo năm
    data_splits = split_data_by_years(df, available, predictor)
    ...
    # ==================== Step 1: Pretrain trên 10 năm ====================
    print(f"\n{'='*60}")
    print("Step 1: Pretrain trên 2015-2025 (10 năm)")
    print(f"{'='*60}")
    ...
    print(f"\n{'='*60}")
    print("Step 2: Fine-tune trên 2021-2025 (4 năm) - Freeze Encoder")
    print(f"{'='*60}")
    ...
    torch.save(artifact, 'nvda_lstm_v7_pretrain_finetune_artifact.pth')
    print(f"\n{'='*60}")
    print("Artifact saved: nvda_lstm_v7_pretrain_finetune_artifact.pth")
    print(f"{'='*60}")
